...
|link:api-management-platform-for-sap.adoc[API management platform for SAP]
|Integrate SAP and non-SAP applications seamlessly with an API first approach, adopting cloud native and microservices development
patterns for new applications and stop developing inside SAP so that a basic core is kept, which dramatically speeds up and
simplifies the upgrades.
...
|link:healthcare.adoc[Healthcare collection]
...
|link:industtrial-edge.adoc[Industrial Edge]
...
|link:data-synthesis.adoc[Modernizing testing with data synthesis]
