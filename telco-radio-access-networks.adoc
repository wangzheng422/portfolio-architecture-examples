...
Radio access network (RAN) is the part of a mobile network that connects end-user devices, like smartphones, to the cloud. For telecommunications network operators, RANs account for significant overall network expenses and key focus area as more edge and 5G use cases emerge for telco customers.
...
Central data center hosts 5G core components and others management functions (cluster management, code/configuration repository) etc.
...
The various components of RAN solution are orchestrated in a standardized manner using Kubernetes primitives and ACM. The event streaming data for various metrics and logs is enabled with Kafka.
